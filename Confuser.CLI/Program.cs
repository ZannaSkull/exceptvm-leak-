...
";
			Console.Title = "ExceptVM";
			WriteLineWithColor(ConsoleColor.Red, Ascii);
			...
			try {
				...
				try {
					...
					Console.Write("ExceptVM Console: ");
				...
				if (files.Count == 1 && Path.GetExtension(files[0]) == ".crproj") {
					...
					if (string.IsNullOrEmpty(outDir)) {
						Console.WriteLine("ExceptVM Console: No output directory specified.");
						PrintUsage();
						return -1;
					}
		...
		static int RunProject(ConfuserParameters parameters) {
			...
			Console.Title = "ExceptVM - Running...";
		...
		class ConsoleLogger : ILogger {
			...
			public void Finish(bool successful) {
				...
				if (successful) {
					Console.Title = "ExceptVM - Success";
					WriteLineWithColor(ConsoleColor.Green, "Finished " + timeString);
					WriteLineWithColor(ConsoleColor.White, "Press any key to contiune...");
					Console.ReadKey();
					ReturnValue = 0;
				}
				else {
					Console.Title = "ExceptVM - Fail";
